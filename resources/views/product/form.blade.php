        ...
        <div class="card shadow mb-4 mt-4">
            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <select multiple name="idr_size[]" id="idr_size" class="form-control select2-basic-single @error('idr_size[]') is-invalid @enderror" aria-required="true" aria-invalid="false">
                        <option value="">-- Pilih Data --</option>
                        @foreach ($size as $res)
                        @php
                        if(isset($product)){
                            $split = str_replace(","," ", $product->idr_size);
                            $explode = explode(',', $product->idr_size);
                        }
                        @endphp
                        <option value="{{ $res->idr_size }}" {{ old('idr_size', isset($product) ? preg_match("/$res->idr_size/i",$split) : '') == $res->idr_size ? 'selected' : '' }}>{{ $res->name }}</option>

                            {{-- @if(in_array($res->idr_size, $product->idr_size))
                            <option value="{{ $res->idr_size }}" selected="true">{{ $res->name }}</option>
                            @else
                            <option value="{{ $res->idr_size }}">{{ $res->name }}</option>
                            @endif  --}}
...
<script>
    ...
    @if(isset($product))
        ...
        var sel = [{{ $product->idr_size }}];
        //console.log({{$product}});
        $("#idr_size").select2().val(sel).trigger('change.select2');
